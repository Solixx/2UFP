 ...
 *  @author Kevin Wayne
 */
public class LinearProbingHashST_AED2_UFP<Key, Value> {
     ...
     */
    public LinearProbingHashST_AED2_UFP() {
        this(INIT_CAPACITY);
    }
     ...
     */
    public LinearProbingHashST_AED2_UFP(int capacity) {
        m = capacity;
        n = 0;
    ...
    private void resize(int capacity) {
        LinearProbingHashST_AED2_UFP<Key, Value> temp = new LinearProbingHashST_AED2_UFP<Key, Value>(capacity);
        for (int i = 0; i < m; i++) {
            if (keys[i] != null) {
                temp.put(keys[i], vals[i], false);
            }
        }
        keys = temp.keys;
        vals = temp.vals;
     ...
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public void put(Key key, Value val, boolean test_resize) {
        if (key == null) throw new IllegalArgumentException("first argument to put() is null");

        if (val == null) {
            delete(key, test_resize);
            return;
        }

        // double table size if 50% full
        if (n >= m/2 && test_resize) resize(2*m);
     ...
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public void delete(Key key, boolean test_resize) {
        ...
        while (keys[i] != null) {
            ...
            vals[i] = null;
            n--;
            put(keyToRehash, valToRehash, test_resize);
            i = (i + 1) % m;
        }
        ...
        if (n > 0 && n <= m/8 && test_resize) resize(m/2);
